...
impl Demistify for ParserError {
    fn demistify(&self) -> String {
        match self.code {
            ...
            ParserErrorCode::MissingOpenBraceAfterClassDeclaration => format!("expected '}}' after class declaration{}", self.demistify_next_token()),
            ParserErrorCode::ThisOutsideMethod => "cannot use 'this' outside methods".to_string(),
            ParserErrorCode::InitializerReturnValue => "cannot return value inside initializer".to_string(), // c => format!("missing error demistifyer for {}", c as u32),
        }
    }
}
